...
async function generateSeparatedLayers(ai, reference, request) {
  ...
  const floorPrompt = `${shared} Create the FLOOR LAYER only. Keep only continuous walkable terrain, floor tiles, paths and flat non-occluding ground markings. Remove absolutely all furniture, equipment, plants, props, perimeter walls, foreground walls, windows, doors, roofs, borders and vertical structures. Reconstruct the floor tiles underneath every removed element and extend the walkable floor across the same room footprint. No wall may remain in this layer. Output a complete 1024x768 image, edge to edge.`;
