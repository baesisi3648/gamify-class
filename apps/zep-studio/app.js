(() => {
  ...
  function createLayeredMap(plan){
    ...
    floor.fillStyle=colors.ground;floor.fillRect(0,0,width,height);floor.fillStyle='rgba(255,255,255,.035)';for(let y=0;y<height;y+=32)for(let x=0;x<width;x+=32)if((x/32+y/32)%2===0)floor.fillRect(x,y,32,32);
    const px=v=>planNumber(v,0,100,50)*width/100,py=v=>planNumber(v,0,100,50)*height/100;
    (plan.waters||[]).slice(0,3).forEach(item=>{const x=px(item.x),y=py(item.y),w=px(planNumber(item.w,3,35,16)),h=py(planNumber(item.h,3,35,12));floor.fillStyle=colors.water;floor.beginPath();floor.ellipse(x+w/2,y+h/2,w/2,h/2,0,0,Math.PI*2);floor.fill();floor.strokeStyle='rgba(255,255,255,.32)';floor.lineWidth=3;floor.stroke();collision.fillStyle='rgba(255,53,93,.72)';collision.beginPath();collision.ellipse(x+w/2,y+h/2,w/2,h/2,0,0,Math.PI*2);collision.fill();});
    floor.lineCap='round';(plan.paths||[]).slice(0,8).forEach(path=>{floor.strokeStyle=colors.path;floor.lineWidth=Math.max(10,px(planNumber(path.width,2,15,6)));floor.beginPath();floor.moveTo(px(path.x1),py(path.y1));floor.lineTo(px(path.x2),py(path.y2));floor.stroke();floor.strokeStyle='rgba(255,255,255,.18)';floor.lineWidth=Math.max(2,floor.lineWidth-8);floor.stroke();});
    (plan.trees||[]).slice(0,18).forEach(tree=>{const x=px(tree.x),y=py(tree.y),s=Math.max(12,px(planNumber(tree.size,2,12,5)));objects.fillStyle='#6f4932';objects.fillRect(x-s*.12,y,s*.24,s*.55);objects.fillStyle=colors.tree;[[0,-.25],[-.27,0],[.27,0],[0,.2]].forEach(([dx,dy])=>{objects.beginPath();objects.arc(x+dx*s,y+dy*s,s*.42,0,Math.PI*2);objects.fill();});objects.fillStyle='rgba(255,255,255,.15)';objects.beginPath();objects.arc(x-s*.15,y-s*.22,s*.16,0,Math.PI*2);objects.fill();collision.fillStyle='rgba(255,53,93,.72)';collision.beginPath();collision.arc(x,y+s*.28,s*.24,0,Math.PI*2);collision.fill();});
    (plan.objects||[]).slice(0,14).forEach(item=>{const x=px(item.x),y=py(item.y),s=Math.max(8,px(planNumber(item.size,1,8,3))),type=String(item.type||'bush');objects.save();if(type==='flower'){objects.fillStyle=colors.accent;for(let i=0;i<5;i++){const a=i*Math.PI*2/5;objects.beginPath();objects.arc(x+Math.cos(a)*s*.3,y+Math.sin(a)*s*.3,s*.2,0,Math.PI*2);objects.fill();}}else if(type==='bench'){objects.fillStyle='#7d5234';objects.fillRect(x-s,y-s*.25,s*2,s*.45);objects.fillRect(x-s*.75,y+s*.2,s*.18,s*.55);objects.fillRect(x+s*.57,y+s*.2,s*.18,s*.55);}else if(type==='lamp'||type==='sign'){objects.fillStyle='#4c5264';objects.fillRect(x-s*.12,y-s*.2,s*.24,s*1.25);objects.fillStyle=colors.accent;objects.fillRect(x-s*.45,y-s*.65,s*.9,s*.55);}else{objects.fillStyle=type==='rock'?'#868a91':colors.tree;objects.beginPath();objects.arc(x,y,s*.6,0,Math.PI*2);objects.fill();}objects.restore();collision.fillStyle='rgba(255,53,93,.7)';collision.fillRect(x-s*.35,y-s*.15,s*.7,s*.65);});
    (plan.buildings||[]).slice(0,6).forEach(building=>{const x=px(building.x),y=py(building.y),w=px(planNumber(building.w,8,42,22)),h=py(planNumber(building.h,8,35,18));top.fillStyle='rgba(0,0,0,.2)';top.fillRect(x+6,y+8,w,h);top.fillStyle=colors.wall;top.fillRect(x,y+h*.3,w,h*.7);top.fillStyle=colors.roof;top.beginPath();top.moveTo(x-w*.04,y+h*.35);top.lineTo(x+w*.5,y);top.lineTo(x+w*1.04,y+h*.35);top.closePath();top.fill();top.strokeStyle='rgba(70,38,38,.45)';top.lineWidth=3;top.stroke();top.fillStyle='#5f493c';top.fillRect(x+w*.43,y+h*.62,w*.14,h*.38);top.fillStyle='#8bd0df';for(let i=1;i<=3;i++)top.fillRect(x+w*i/4-w*.055,y+h*.46,w*.11,h*.12);collision.fillStyle='rgba(255,53,93,.72)';collision.fillRect(x,y,w,h);});
    const composite=makeLayer(width,height),cc=composite.getContext('2d');cc.drawImage(canvases.floor,0,0);cc.drawImage(canvases.object,0,0);cc.drawImage(canvases.top,0,0);
    const urls={composite:composite.toDataURL('image/png')};Object.entries(canvases).forEach(([key,value])=>urls[key]=value.toDataURL('image/png'));return urls;
  }
