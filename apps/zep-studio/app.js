(() => {
  ...
  async function createLayeredMap(plan){
    ...
    floor.fillStyle=colors.ground;floor.fillRect(0,0,width,height);for(let y=-16;y<height;y+=138)for(let x=-16;x<width;x+=138)floor.drawImage(sprites[3][0],x,y,154,154);
    const px=v=>planNumber(v,0,100,50)*width/100,py=v=>planNumber(v,0,100,50)*height/100;
    (plan.waters||[]).slice(0,3).forEach(item=>{const x=px(item.x),y=py(item.y),w=px(planNumber(item.w,3,35,16)),h=py(planNumber(item.h,3,35,12));floor.save();floor.beginPath();floor.ellipse(x+w/2,y+h/2,w/2,h/2,0,0,Math.PI*2);floor.clip();floor.fillStyle=colors.water;floor.fillRect(x,y,w,h);for(let ty=y-20;ty<y+h;ty+=100)for(let tx=x-20;tx<x+w;tx+=100)floor.drawImage(sprites[3][2],tx,ty,120,120);floor.restore();collision.fillStyle='rgba(255,53,93,.72)';collision.beginPath();collision.ellipse(x+w/2,y+h/2,w/2,h/2,0,0,Math.PI*2);collision.fill();});
